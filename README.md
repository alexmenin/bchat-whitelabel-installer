...
sudo ./install-online.sh agilizesolucoes/bchat-whitelabel:1.2.8 3388
```
...
sudo git pull
sudo ./bchatctl update-online agilizesolucoes/bchat-whitelabel:1.2.8
```
...
O usuario administrador inicial e `admin@empresa.local` com a senha `admin123`.
Altere a senha imediatamente apos o primeiro acesso. Para definir outros dados
antes da primeira subida, edite `INIT_ADMIN_NAME`, `INIT_ADMIN_EMAIL` e
`INIT_ADMIN_PASSWORD` no `.env`.
...
A versao `1.2.8` remove o codigo de setup e libera o fluxo diretamente pela
ativacao da licenca,
...
Ou publique tudo em uma unica etapa:

```bash
sudo ./bchatctl publish
```
